...
const baseUrl = process.env.URL || "https://www.bigbadcon.com"
...
async function screenshot(url, format, viewportSize, dpr = 1, withJs = false) {
...
//   let zoom = event.queryStringParameters && event.queryStringParameters.zoom
  ...
  if(size === "small") {
    ...
    let url = `${baseUrl}/og/events/${slug}`
  ...
  try {
    ...
    return {
      ...
      body: `<img src="${baseUrl}/static/images/og.jpg" />`,
