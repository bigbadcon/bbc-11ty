...
document.addEventListener('alpine:init', () => {
  ...
  Alpine.data('createAccount',() => ({
    ...
    async checkUsername() {
      try {
        const res = await fetch(`/.netlify/functions/check-user/${this.userNicename}`)
  ...
  Alpine.data('resetPasswordForm',() => ({
    ...
    async resetPassword() {
      ...
      if (this.userEmail) {
        ...
        // TODO: change to fetch
        const res = await fetch(`/.netlify/functions/forgot-password/?email=${paramSafeEmail}`).then(function(response) {console.log(response)})
