...
app.command("/website", async ({ body, ack }) => {
	...
	const adminBuildHookUrl = `https://api.netlify.com/build_hooks/${process.env.NETLIFY_ADMIN_BUILD_HOOK_SLACKBOT}`;
	//  trigger different functions with parameters
	if (body.text === "")
		await sendChat(
			body,
			`Hi ${body.user_name}! I'm the website bot! You can use the following slash commands:\n*/website build* triggers a new production build\n*/website drafts* triggers a new drafts branch build\n*/website publish* publishes the most recent deploy live`
		);
	if (body.text === "build") {
		await sendChat(body, `Build command for Production initiated!`);
		try {
			await axios.post(`${buildHookUrl}?trigger_title=slackbot`, {});
			await axios.post(`${adminBuildHookUrl}?trigger_title=slackbot`, {});
			await sendChat(
				body,
				`Build commencing! It should be ready in a couple minutes at https://www.bigbadcon.com`
			);
		} catch (err) {
	...
	if (body.text === "drafts") {
		...
		try {
			...
			await sendChat(
				body,
				`Build commencing! It should be ready in a couple minutes at https://drafts--hopeful-pike-1a02ec.netlify.app`
			);
		} catch (err) {
			await sendChat(body, `Sorry build command failed for some reason!`);
		}
